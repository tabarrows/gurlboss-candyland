...
import { CANDY_IMAGES, formatPrice, PLATTER_IMAGE } from '@/lib/types';
import { useCart } from '@/lib/cart-context';
import { useSettings } from '@/lib/use-settings';
import { Check, X, ShoppingBag, Star } from 'lucide-react';
import { useNavigate } from 'react-router-dom';

export default function GurlBossFavePage() {
  ...
  useEffect(() => {
    (async () => {
      const [{ data: prods }, { data: cand }] = await Promise.all([
        supabase.from('products').select('*').eq('product_type', 'gbf').eq('is_visible', true).order('sort_order'),
  ...
  const toggleCandy = (name: string) => {
    ...
    if (selections.includes(name)) {
      ...
      if (selections.length >= maxChoices) {
        setError(`You can select up to ${maxChoices} candies for this platter size.`);
        return;
      }
  ...
  const handleAddToCart = () => {
    ...
    if (selections.length === 0) {
      setError('Please select at least one candy for your platter.');
      return;
    }
    if (selections.length > maxChoices) {
    ...
    const item: CartItem = {
      ...
      image_url: currentProduct.image_url || PLATTER_IMAGE,
  ...
  return (
    <div className="animate-fade-in">
      {/* Hero */}
      <div className="relative overflow-hidden bg-gradient-to-br from-gcl-pink-100 via-white to-gcl-pink-50">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-12">
          <div className="grid lg:grid-cols-2 gap-8 items-center">
            <div>
              <span className="inline-flex items-center gap-2 rounded-full bg-gcl-red-100 px-4 py-1.5 text-xs font-semibold text-gcl-red-600 mb-3">
                <Star className="h-3.5 w-3.5" />
                Signature Product
              </span>
              <h1 className="font-display text-3xl lg:text-4xl text-gcl-pink-600 mb-3">
                GurlBoss Fave — Candy Platter
              </h1>
              <p className="text-gray-600 mb-4">
                The GBF is a real candy platter — a beautiful tray loaded with your choice of loose assorted candy, arranged to perfection. Not a bag, not a box — a true platter experience.
              </p>
              <div className="flex flex-wrap gap-2">
                {products.map((p) => (
                  <span key={p.id} className="badge bg-gcl-pink-100 text-gcl-pink-600 px-3 py-1.5">
                    {p.size_label}
                  </span>
                ))}
              </div>
            ...
            <div className="relative rounded-3xl overflow-hidden shadow-2xl aspect-[4/3]">
              ...
              <div className="absolute bottom-4 left-4 right-4 bg-white/90 backdrop-blur-sm rounded-xl p-3 text-center">
                <p className="text-sm font-bold text-gcl-pink-600">A Real Candy Platter</p>
                <p className="text-xs text-gray-500">Beautifully arranged on a tray</p>
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-8">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            {products.map((p) => (
              ...
              >
                ...
                <p className="text-xs text-gray-500 mt-1">Up to {p.max_candy_choices} candy choices</p>
          ...
          <div className="animate-slide-up">
            <div className="flex items-center justify-between mb-4 flex-wrap gap-2">
              <h2 className="text-lg font-bold text-gray-900">
                ...
                Choose Candies for Your Platter
              </h2>
            ...
            <div className="mb-6">
              ...
              <p className="text-xs text-gray-400 mt-1">
                {selections.length === 0
                  ? `Select up to ${maxChoices} candies for your platter`
            ...
            {selections.length > 0 && (
              <div className="mt-8 rounded-2xl bg-gcl-pink-50 border border-gcl-pink-100 p-5">
                <h3 className="text-sm font-bold text-gcl-pink-600 mb-3">Your Platter Selection</h3>
                <div className="flex flex-wrap gap-2">
                  {selections.map((c, i) => (
                    <span
                      key={i}
        ...
        {!selectedSize && !loading && (
          <div className="text-center py-16">
            <Star className="h-12 w-12 text-gcl-pink-200 mx-auto mb-4" />
            <p className="text-gray-500">Select a platter size above to start choosing your candies!</p>
          </div>
        )}
      </div>
    </div>
